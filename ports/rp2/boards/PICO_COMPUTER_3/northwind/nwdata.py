# Northwind-lite -- schema and seed data for the Pico Computer 3 SQL demo.
#
# A small approximation of Microsoft's classic "Northwind" sample database:
# a fictional food-importer's customers, products and orders. The row counts
# are trimmed to a few dozen so the whole thing builds in a second and lives
# happily in the PSRAM heap, but the eight-table shape is the real thing, so
# the joins in nwquery.py look like textbook Northwind queries.
#
# The data here is plain Python -- lists of tuples -- so this module imports
# on a desktop too. build(con) takes an already-open connection, which is why
# it works identically against on-device `usqlite` and desktop `sqlite3`
# (handy for testing). Everyday use goes through open_db() at the bottom.

# --- schema -----------------------------------------------------------------
# One CREATE per table, parent tables before the children that reference them.
SCHEMA = (
...
# A couple of indexes on the foreign keys the reports group and join by.
INDEXES = (
...
# --- seed data --------------------------------------------------------------
# Names are the real Northwind ones, de-accented so they render cleanly in the
# machine's bitmap fonts (the database itself is happy with UTF-8 either way).

CATEGORIES = (
    (1, "Beverages", "Soft drinks, coffees, teas, beers and ales"),
    (2, "Condiments", "Sweet and savoury sauces, relishes, spreads"),
...
CUSTOMERS = (
    ...
    ("GODOS", "Godos Cocina Tipica", "Jose Pedro Freyre", "Seville", "Spain"),
)
...
# Orders: ID, customer, employee, date (ISO so it sorts and date()s cleanly),
# shipper. Order numbers start at 10248 as they do in the real Northwind.
ORDERS = (
...
# Order lines: order, product, unit price charged, quantity, discount (0..1).
# Prices mostly match the product's list price; a couple differ, as they can.
ORDERDETAILS = (
...
# Bound to their table + column list for a tidy executemany() loop in build().
SEED = (
    ("Categories", "CategoryID, CategoryName, Description", CATEGORIES),
    ("Suppliers", "SupplierID, CompanyName, City, Country", SUPPLIERS),
    ("Shippers", "ShipperID, CompanyName", SHIPPERS),
    ("Employees", "EmployeeID, LastName, FirstName, Title", EMPLOYEES),
    ("Customers", "CustomerID, CompanyName, ContactName, City, Country", CUSTOMERS),
    ("Products", "ProductID, ProductName, SupplierID, CategoryID, UnitPrice, UnitsInStock", PRODUCTS),
    ("Orders", "OrderID, CustomerID, EmployeeID, OrderDate, ShipVia", ORDERS),
    ("OrderDetails", "OrderID, ProductID, UnitPrice, Quantity, Discount", ORDERDETAILS),
)
...
def build(con):
    """Create every table + index and load the seed data on the open connection
    `con`. Everything happens inside one transaction, so a build either lands
    whole or not at all -- and a single COMMIT is far faster than committing
    every one of the ~150 inserts separately on the flash disk.

    Note: we insert one execute()-per-row rather than executemany(). usqlite's
    executemany() runs a multi-statement SQL *script* (it takes no parameter
    list), so parameterised bulk inserts go through execute() in a loop -- which
    behaves identically on usqlite and on desktop sqlite3."""
    con.execute("BEGIN")
    for stmt in SCHEMA:
        con.execute(stmt)
    for table, cols, rows in SEED:
        sql = "INSERT INTO %s (%s) VALUES (%s)" % (table, cols, _placeholders(cols))
        for row in rows:
            con.execute(sql, row)
    for stmt in INDEXES:
        con.execute(stmt)
...
def _built(con):
    """True if the database already has our tables (so we don't rebuild)."""
    cur = con.execute(
...
def open_db(path="/sd/northwind.db", force=False):
    """Open (and if necessary build) the Northwind database, returning the live
    connection. First call on a fresh machine creates and seeds the file, which
    then survives a power-cycle; later calls just open it. Pass force=True to
    wipe and rebuild."""
    import usqlite  # on-device SQLite; imported lazily so this file loads on a PC

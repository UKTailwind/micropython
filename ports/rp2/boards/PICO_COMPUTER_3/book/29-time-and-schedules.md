# Chapter 29 — Time and schedules: the real-time clock
...
needed by hand; and chapter 30 sets the whole thing from the internet
in one line.)
...
![The machine's three clocks — the battery DS3231, the system clock it seeds at boot, and the millisecond ticker — and the eight-number tuple `gettime()` returns: positions 0–5 the date and time, 6 the weekday (0 = Mon), 7 the day of the year.](figs/29-clocks.png)
...
4. **The hardware alarm.** Your software alarm dies with its program;
   ...
   waking code with no polling loop at all — is chapter 34's

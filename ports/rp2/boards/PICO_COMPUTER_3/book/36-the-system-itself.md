...
![The boot sequence, colour-coded by whose code runs each stage: the chip's ROM, the MicroPython firmware, the board's hook, then your `/boot.py` and `/main.py` — or the `>>>` prompt when there is no `main.py`.](figs/36-boot.png)
...
![Where everything lives. The 16 MB flash holds the firmware (replaced on reflash) and your 12 MB `/` filesystem (kept); the SD card is removable; the PSRAM heap vanishes at power-off. Everything precious lives in `/`.](figs/36-flash.png)
...
screen mode and clock, the RGB1024 palette, and chapter 30's Wi-Fi
credentials, timezone and auto-sync flag — updated automatically by
`keymap()`, `screen()`, `palette()`, `wifi()`, `tz()`, `auto()`.
It's JSON (chapter 30 taught you to read it: `cat("/settings.json")`),
...
Run it weekly (or wire it to chapter 29's cron-junior and never think
...
- `boards/PICO_COMPUTER_3/` holds the Python you have been using all
  ...
  `pcsprite.py`, `pcgui.py`, `ds3231.py` (chapter 32 already sent
...
- The C floor — `hdmi.c` and friends — is where chapter 35's ladder
...
- **Sideways to big Python** — the language you know *is* Python.
  ...
  (chapter 31) was chosen to match the NumPy you'd meet there.
...
2. Time the boot: power-switch to prompt, phone stopwatch. Now add
   `auto(True)` (chapter 30) and time it again — the cost of a
...
2. **The heirloom backup.** `backup.py` grown up: recurses into
   ...
   chapter 34's uptime discipline applied to *data*.

...
`patch(1)` is a build dependency since the TinyUSB 0.21 move (§73): one
TinyUSB file is patched into the build directory at build time. `lib/tinyusb`
must be at the commit this branch pins (`git submodule update --init
lib/tinyusb`); the configure step refuses any TinyUSB that is not 0.21.
...
### 73. TinyUSB 0.21 — the host stack follows upstream v1.29.0

**Why.** MicroPython v1.29.0 pins `lib/tinyusb` at `b549ac1d8` =
`0.21.0-micropython1` (upstream moved to 0.21.0 in June 2026, then to their own
fork `micropython/tinyusb` in August). Our v1.29.0 merge kept the older parity
pin, 0.20.0 — so v0.16 shipped v1.29.0's `shared/tinyusb` glue on a TinyUSB one
release behind what upstream tested it with. More to the point, 0.20's rp2 host
controller driver is the one that took the Fuzix kernel down in the field:
`need_pre()` asks the root port its speed at **every transfer start** and
`hcd_port_speed_get()` panics ("Invalid speed") if it reads 0 — which it does
for the moment a hub and keyboard, powering up together, drop the link while
enumerating. The same file is compiled into this firmware for the keyboard; under
the SDK's stock `panic()` it would present as an occasional hard fault at
power-on with a keyboard attached. 0.21 rewrote that driver (EPX switching on
the RP2350's `STOP_EPX_ON_NAK`, a 300 µs NAK poll instead of 16 µs,
`TUSB_SPEED_INVALID` instead of the panic) and made the host stack asynchronous.

**What changed.**

- `.gitmodules` → `https://github.com/micropython/tinyusb.git`, `lib/tinyusb`
  → `b549ac1d8` (`0.21.0-micropython1`): exactly what v1.29.0 pins.
- `ports/rp2/CMakeLists.txt`: with `MICROPY_HW_USB_HOST`, the configure step
  reads `TUSB_VERSION_MINOR` from `lib/tinyusb/src/tusb_option.h` and refuses
  anything but 21 (the submodule drifted under a merge once; it must not do so
  silently again). Then it drops the SDK's `hcd_rp2040.c` from
  `tinyusb_host_base`'s sources and compiles a copy generated by `patch(1)`
  from **`ports/rp2/hcd_rp2040.patch`** into the build directory. If TinyUSB
  changes under the hunk, `patch` refuses and the build stops there.
- **The patch.** 0.21 as shipped does not boot an RP2350 host behind a hub: its
  RX-timeout handler fails the stopped transfer (`xfer_complete_isr(...FAILED)`)
  without clearing the EPX buffer it had armed; `usbh` retries the request and
  the retry's first buffer arm hits `bufctrl_write32`'s "buf_ctrl already
  available" — a panic — on the status-stage IN of an enumeration request.
  One line, `usbh_dpram->epx_buf_ctrl = 0;` before the failure completion, as
  the driver's own EPX-switch path already does. Found on the Fuzix kernel
  (2026-09-01, `PC3-IRQ-REVIEW.md` there) with a panic hook that printed the
  register; both firmwares now carry the identical patch.
- `mp_usbh.c`: `tuh_init(0)` → `tuh_rhport_init(0, &{TUSB_ROLE_HOST,
  TUSB_SPEED_AUTO})`. 0.21 keeps `tuh_init()` only as a *deprecated* inline
  wrapper, and this port builds with `-Werror`, so the first build stopped on
  exactly that line (`-Werror=deprecated-declarations`). Then set
  `LINESTATE_TUNING.MULTI_HUB_FIX` — the RP2350's own fix for turnaround
  timeouts through a hub; the on-board hub is always in the path here.
- Not changed, worth knowing: 0.21 adds `CFG_TUH_HID_SET_PROTOCOL_ON_ENUM`
  (default 1) — boot protocol on boot-capable interfaces at enumeration, which
  is the behaviour §13's "do NOT call `tuh_hid_set_protocol(BOOT)`" comment
  already relied on; `usb_touch.c` switches its interfaces to report protocol
  itself, as before.

**Status.** Built (`make BOARD=PICO_COMPUTER_3`, zero warnings; the patched
`hcd_rp2040_pc3.c.o` is the only hcd in the link; text +568 bytes over the 0.20
build) and flashed to a PC3 on 2026-09-01. Runtime proof it is this code:
`os.uname().version` names the commit, and `machine.mem32[0x50110088]`
(`LINESTATE_TUNING`) reads `0xfc` — the reset value `0xf8` with bit 2,
`MULTI_HUB_FIX`, set. Trap met on the way: straight after a BOOTSEL flash the
USB switch is still in the *Prog* position, so `SIE_STATUS` (`0x50110050`)
reads `0x1` — VBUS only, nothing on the root port, not even the hub — and no
keyboard can attach until the switch is moved.

**Seen on the board (2026-09-01, Corsair keyboard `1b1c:1b3d`).** Switch to the
hub, keyboard hot-plugged: `SIE_STATUS 0x50001205` (full speed, live bus),
`INT_EP_CTRL 0x6` — two interrupt pipes, this keyboard's two HID interfaces.
Then `machine.reset()` with it attached, the case the field report was about:

```
USB HID: addr=1 instance=0 protocol=1
USB keyboard -> slot 1
USB HID: addr=1 instance=1 protocol=0
USB HID: ignored addr=1 inst=1 VID:PID=1b1c:1b3d (not a gamepad)
USB: mounted addr=1 VID:PID=1b1c:1b3d
USB: unmounted addr=1
USB HID: addr=1 instance=0 protocol=1
USB keyboard -> slot 1
USB: mounted addr=1 VID:PID=1b1c:1b3d
```

Mount, drop, re-mount, keyboard back in slot 1, REPL alive — the same
`attached / detached / attached` the Fuzix kernel prints on 0.21, i.e. the
link really does drop once while the keyboard comes up after a reset, and the
stack now re-enumerates instead of panicking. Still owed: every host class this
port has and the kernel does not —
mouse (`mouse()`), touch (`touch()`, including the report-protocol switch and
the bring-up handshake), gamepad, `USBSerial` — and the num-lock LED push
(§72), all of which sit on the request-based polling that 0.21's asynchronous
...
| `.gitmodules` | register `lib/usqlite` submodule (url = UKTailwind/usqlite, branch pc3-micropython-1.29); `lib/tinyusb` url → `micropython/tinyusb.git` as upstream v1.29.0 has it (§73) |

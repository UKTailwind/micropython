...
Computer 2 — version **0.17** (test release).
...
```
MicroPython v1.29.0 on PICO COMPUTER 2 v0.17 with RP2350B
```
...
out on core 1. Six modes are available:
...
| `hdmi.RGB320_8` | 320 × 240 | 8-bit (256 colours), pixel-doubled to 640 × 480 — one buffer is only a **quarter** of the video SRAM, so the display, the **layer** *and* the `create()` F buffer all fit in fast SRAM **together**: the only mode with an overlay and a fast double buffer at the same time |

The framebuffer lives in SRAM. In `RGB640` and `RGB320_8` each pixel is one byte
(RGB332); in the two 16-bit modes each pixel is an RGB565 half-word; in the 4-bit
modes two pixels share a byte (palette index). `RGB1024` gives a true, crisp 1024 × 600 —
...
The optional `clock` is the CPU/pixel clock and applies to the **640 × 480-timing
modes** (`RGB640`, `RGB320`, `RGB640_4`, `RGB320_8`), which accept one of **252**,
**315** or **378** MHz (default 252). `RGB512` and `RGB1024` are fixed at
252 MHz — passing any other clock for them raises a `ValueError`. The chosen mode/clock is saved and restored on the next boot
...
| Member | Description |
|---|---|
| `hdmi.RGB640` / `RGB320` / `RGB512` / `RGB1024` / `RGB640_4` / `RGB320_8` | mode constants (see the table above) |
...
| `hdmi.rgb565()` | `True` if the framebuffer is 16-bit RGB565 (RGB320/RGB512); `False` for RGB332 (RGB640/RGB320_8) or the 4-bit modes |
| `hdmi.bpp()` | bits per framebuffer pixel: `4` (RGB1024/RGB640_4), `8` (RGB640/RGB320_8) or `16` (RGB320/RGB512) |
...
| `"L"` | the **L**ayer — an overlay merged over the display live, **RGB320 and RGB320_8 modes** |
| `"F"` | an off-screen **F**ramebuffer — draw or decode into it invisibly, then copy |

| Command | Description |
|---|---|
| `hdmi.layer(transparent=0x000000)` | enable the layer (RGB320/RGB320_8). `transparent` is an RGB888 colour; layer pixels of that colour show the display through, anything else overlays it. The layer starts fully transparent. (In RGB320_8 the colour is matched at RGB332 resolution) |
| `hdmi.create()` | allocate the off-screen F buffer (display-sized). In `RGB640_4` and `RGB320_8` — and in `RGB320` when no layer exists — it lives in the free video SRAM (much faster than PSRAM); otherwise PSRAM |
...
The layer lives in the video memory directly after the framebuffer (in RGB320
2 × 320×240×16-bit exactly fills it; in RGB320_8 it is the second quarter), so
it costs no extra RAM and the merge happens **per scanline in hardware-speed C
on core 1**: moving a sprite on the layer never disturbs the artwork
underneath. A mode change closes both targets.

In RGB320 the free half is shared, first come first served: `hdmi.create()`
...
**RGB320_8 has no such rivalry**: one buffer is a quarter of the video SRAM, so
the layer owns the second quarter and the F buffer the third — `hdmi.layer()`
and `hdmi.create()` always succeed together, in either order, all in fast SRAM.
It is the mode to pick when a game wants a scenery-preserving overlay *and*
flicker-free double buffering at full 256-colour depth.
...
returns it. Any standard USB HID mouse works — the report layout (button
count, 8/12/16-bit movement, wheel) is read from the mouse's own descriptor.
...
hdmi.layer(); hdmi.write("L")   # overlay layer (RGB320/RGB320_8): sprites over scenery
...
*Pico Computer 3 firmware v0.17 — based on MicroPython. See

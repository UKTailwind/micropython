# Chapter 34 — Doing several things at once
...
Chapter 32 polled its button sixty times a second. The interrupt way
inverts it: the *pin* calls *you*.

![An interrupt is a tap on the shoulder: when the pin (or timer) fires, its handler runs between two statements of your main program, then the program resumes exactly where it paused.](figs/34-interrupt.png)

Wire the chapter 32 button (GP1 to
...
Chapter 29's hardware alarm, completed. The chip decides *when*; the
...
![`asyncio` shares one core between many tasks. Each runs until its `await`, then hands the core to the next ready task — cooperative, so tasks share variables without locks, but any task that never awaits freezes them all.](figs/34-asyncio.png)
...
chapter 17's bouncing ball, animating *while* chapter 30's weather
...
1. **The morning machine.** Chapter 29's bedside clock, rebuilt on

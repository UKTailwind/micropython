# Chapter 32 — Standing on others' shoulders
...
at the raw file. Wi-Fi up (chapter 29), then:
...
Chapter 31 met the TSL2591 light sensor on the QWIIC socket and read it
...
chapter 31), note the address, search "micropython" and the sensor's
...
first thing your `/main.py` runs (chapter 35) and the machine greets
...
1. **The card, completed.** Add today's date (the DS3231, chapter 28)
   and the machine's free memory (`gc.mem_free()`, chapter 34) to the
...
2. **The tracker.** A GPS position logged to a CSV file once a minute
   (chapter 28's cron-junior), then drawn as a path with `plot()`
   (chapter 30). A walk around the block, saved as data.
